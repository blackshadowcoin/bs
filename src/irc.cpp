...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("BlackShadowCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BlackShadowCoinTEST2\r");
            Send(hSocket, "WHO #BlackShadowCoinTEST2\r");
        } else {
            // randomly join #BlackShadowCoin00-#BlackShadowCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BlackShadowCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BlackShadowCoin%02d\r", channel_number).c_str());
        }
