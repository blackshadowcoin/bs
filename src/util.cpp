...
#else
    const char* pszModule = "BlackShadowCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BlackShadowCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BlackShadowCoin
    // Mac: ~/Library/Application Support/BlackShadowCoin
    // Unix: ~/.BlackShadowCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BlackShadowCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "BlackShadowCoin";
#else
    // Unix
    return pathRet / ".BlackShadowCoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "BlackShadowCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "BlackShadowCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BlackShadowCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("BlackShadowCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
